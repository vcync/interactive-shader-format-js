...
const ConvertFragment = function ConvertFragment(fragShaderArg) {
  let fragShader = fragShaderArg;
  ...
  if (meta.PASSES) {
    meta.PASSES.map((passArg) => {
      const pass = { ...passArg };

      if (persistentBufferNames.indexOf(pass.TARGET) !== -1) {
...
const ConvertVertex = function ConvertVertex(vertShaderArg) {
  let vertShader = vertShaderArg.replace(
    /vv_vertShaderInit/g,
    "isf_vertShaderInit"
  );
